 ...
 */
export async function analyzePosition(fen, multiPv = 1) {
    try {
        const response = await fetch(`${LICHESS_API_BASE}/cloud-eval?fen=${encodeURIComponent(fen)}&multiPv=${multiPv}`, {
            headers: {
        ...
        const evaluations = data.pvs.map((pv) => ({
            cp: pv.cp,
            mate: pv.mate,
            depth: data.depth || 20,
